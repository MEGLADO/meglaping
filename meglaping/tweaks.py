...
so reverting restores the user's machine rather than someone else's idea of stock.
"""
...
class Tweak:
    ...
    def apply(self, ctx: Ctx) -> tuple[bool, str, str | None]:
        """Returns (ok, message, prior_value) — prior_value goes in the journal."""
        prior = self.read(ctx)
        if prior is None:
...
class FlowControl(NicTweak):
    ...
    detail = "lets your router pause all your traffic instead of dropping one packet. the pause stalls the game too."
...
def checks(ctx: Ctx, counters: dict | None = None, mtu: int | None = None) -> list[Finding]:
    ...
    if counters:
        ...
        out.append(_finding(
            ...
            "packets are being lost at the cable or card, not on the internet. check the cable and the driver." if bad else "",
