...
def cmd_measure(args) -> int:
    ...
    if any(not r.alive for r in regions):
        console.print("[dim]'no reply' usually means that server ignores ping, not that it is down.[/]")
...
def _score_panel(result: scoring.Score, bloat=None, mtu=None) -> Panel:
    ...
    if bloat and bloat.ok:
        ...
        if bloat.delta_ms > 50:
            extras.append("[yellow]Heavy bufferbloat. This is your router's queue, not a Windows setting. Enable SQM/QoS on the router.[/]")
    if mtu:
...
def main(argv: list[str] | None = None) -> int:
    ...
    if not argv:
        # No arguments: open the full-screen app rather than printing one report.
        from .app import run
