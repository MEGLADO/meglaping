...
Uses Win32 IcmpSendEcho. The output of ping.exe and tracert is localized, and a raw
socket would need admin rights.
"""
...
class Pinger:
    """Reusable ICMP handle. Create one per measurement run and share it."""
    ...
    def ping(
        ...
        # here and keep the kernel value as a floor.
...
def measure_many(
    ...
    """Ping several targets concurrently. Handles are not shareable, so each thread makes one.
...
def bufferbloat(ip: str, seconds: int = 10, streams: int = 4, url: str = _LOAD_URL) -> BufferbloatResult:
    ...
    def puller() -> None:
        try:
            ...
            pass  # a dead stream just means less load, and bytes_pulled reports it
