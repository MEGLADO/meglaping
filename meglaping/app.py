...
from . import host, netprobe, rlgame
from . import score as scoring
from . import tweaks
from .tweaks import ACTION, HIGH, INFO, LOW, MEDIUM, OK, UNSUPPORTED

# Real colours, not theme names: markup tags must be valid styles or rendering raises.
...
class MeglaPing(App):
    ...
    async def _show_scan(self, ctx: tweaks.Ctx, findings: list[tweaks.Finding]) -> None:
        ...
        # Everything fixable starts picked; space or click unpicks a row.
        ...
        if todo:
            await body.mount(Label(
                f"{len(todo)} fixable. click a row or press space to unpick it, then press fix.",
                classes="callout",
            ))
        else:
    ...
    def _pick_mark(self, f: tweaks.Finding) -> str:
        if not (f.actionable and f.id in tweaks.BY_ID):
            return " "
    ...
    def _toggle(self, finding_id: str, table: DataTable) -> None:
        ...
        if not finding or not finding.actionable or finding_id not in tweaks.BY_ID:
            return
